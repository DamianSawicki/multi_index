...
int main()
{
  ...
  for(int j = -1; j < 11; ++j) {
    ...
    for(long long iters = 100; iters <= 1'000'000; iters *= 100){
      ...
      for(int k = 0; k < loops; ++k){
        ...
        for(long long i = 0; i < iters / loops; ++i) {
          long long count = es.get<age>().count(i % max_age);
        ...
        for(long long i = 0; i < iters / loops; ++i) {
          long long count = ers.get<age>().count(i % max_age);
          if (count < 1) // To prevent compiler optimisations.
            std::cout << count << std::endl;
        }
      ...
      std::cout << std::fixed << std::setprecision(2) << std::setw(20)
                << (durMicro1 * 1.0)/durMicro2 << " - ratio." << std::endl;
    }
  }
